...
x_values=np.arange(len(filtered_data))*0.03125
plt.plot(x_values, filtered_data)
plt.xticks(np.arange(0,0.0625), fontsize=8)
plt.xlabel('Time (s)')
plt.ylabel('Filtered Values')
plt.show()

N=1000
X = np.fft.fft(filtered_data, n=N)
...
f, psd = welch(filtered_data, fs=fs1, nperseg=1000, scaling='density')
plt.semilogy(f, psd)
plt.xlabel('Frequency (Hz)')
plt.ylabel('PSD')
plt.show()
